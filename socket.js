...
io.sockets.on('connection', function(socket) {
  ...
  socket.on('newUser', function(nickname) {
    ...
    /* 모든 소켓에게 전송 */
    io.sockets.to(roomId).emit('update', {type: 'connect', nickname: 'SERVER', message: nickname + '님이 접속하였습니다.'})
  })
  ...
  socket.on('disconnect', function() {
    ...
    socket.to(roomId).emit('update', {type: 'disconnect', nickname: 'SERVER', message: socket.nickname + '님이 나가셨습니다.'});
